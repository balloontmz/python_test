...
class User(object):
    def __init__(self, usr, pw):
        ...
        self.password = get_md5(pw + self.salt)

db = {}

def register(usr, pw):
    db[usr] = User(usr, pw) # db 可去掉
    with open('/home/tomtiddler/Documents/python_test/tom.txt', 'a') as f:
        json.dump(db[usr], f, default=lambda obj: obj.__dict__)
        f.write('\n') # 罪魁祸首，打开tom情况下运行会多一个'\n'导致读取时无法判定为json格式


def login(usr, pw):
    with open('/home/tomtiddler/Documents/python_test/tom.txt', 'r') as f:
        for lines in f.readlines():
            user = json.loads(lines.strip()) # 提取json文件作为user属性的dict
            if user['usr'] == usr:
                break
    return user['password'] == get_md5(pw + user['salt'])
